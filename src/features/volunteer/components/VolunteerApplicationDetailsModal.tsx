    ...
    <div className="event-request-details-modal" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section aria-busy={isLoading} aria-labelledby="volunteer-application-details-title" aria-modal="true" className="event-request-details-modal__dialog" role="dialog" tabIndex={-1}>
        ...
        {application ? <footer className={`event-request-details-modal__actions ${application.status === "pending" ? "event-request-details-modal__actions--pending" : "volunteer-details__actions--final"}`}>
          {application.status === "pending" ? <><button aria-busy={isSubmitting} className="event-request-details-modal__action event-request-details-modal__action--reject" disabled={isSubmitting} type="button" onClick={() => onReject(reviewNote)}>{isSubmitting ? <span aria-hidden="true" className="volunteer-details__action-spinner" /> : null}{details.reject}</button><button aria-busy={isSubmitting} className="event-request-details-modal__action event-request-details-modal__action--approve" disabled={isSubmitting} type="button" onClick={() => onApprove(reviewNote)}>{isSubmitting ? <span aria-hidden="true" className="volunteer-details__action-spinner" /> : null}{details.approve}</button></> : <span className={`event-request-details-modal__final-status event-request-details-modal__final-status--${application.status ?? "unknown"}`}>{statusLabel}</span>}
        </footer> : null}
      </section>
    </div>
  );
